/**
 * @file sha256.c
 * @brief SHA-256 Implementation per FIPS 180-4
 *
 * SHA-256 produces a 256-bit hash from arbitrary-length input.
 * Key properties:
 * - Deterministic: Same input always produces same output
 * - Avalanche effect: 1-bit change causes ~50% output bit changes
 * - Pre-image resistant: Cannot find input from hash
 * - Collision resistant: Cannot find two inputs with same hash
 */

#include "sha256.h"
#include <string.h>

/* SHA-256 constants K[0..63]: first 32 bits of fractional parts of cube roots of first 64 primes */
const uint32_t sha256_k[64] = {
    0x428a2f98, 0x71374491, 0xb5c0fbcf, 0xe9b5dba5,
    0x3956c25b, 0x59f111f1, 0x923f82a4, 0xab1c5ed5,
    0xd807aa98, 0x12835b01, 0x243185be, 0x550c7dc3,
    0x72be5d74, 0x80deb1fe, 0x9bdc06a7, 0xc19bf174,
    0xe49b69c1, 0xefbe4786, 0x0fc19dc6, 0x240ca1cc,
    0x2de92c6f, 0x4a7484aa, 0x5cb0a9dc, 0x76f988da,
    0x983e5152, 0xa831c66d, 0xb00327c8, 0xbf597fc7,
    0xc6e00bf3, 0xd5a79147, 0x06ca6351, 0x14292967,
    0x27b70a85, 0x2e1b2138, 0x4d2c6dfc, 0x53380d13,
    0x650a7354, 0x766a0abb, 0x81c2c92e, 0x92722c85,
    0xa2bfe8a1, 0xa81a664b, 0xc24b8b70, 0xc76c51a3,
    0xd192e819, 0xd6990624, 0xf40e3585, 0x106aa070,
    0x19a4c116, 0x1e376c08, 0x2748774c, 0x34b0bcb5,
    0x391c0cb3, 0x4ed8aa4a, 0x5b9cca4f, 0x682e6ff3,
    0x748f82ee, 0x78a5636f, 0x84c87814, 0x8cc70208,
    0x90befffa, 0xa4506ceb, 0xbef9a3f7, 0xc67178f2
};

/* Initial hash values H0..H7: first 32 bits of fractional parts of square roots of first 8 primes */
static const uint32_t sha256_initial_state[8] = {
    0x6a09e667, 0xbb67ae85, 0x3c6ef372, 0xa54ff53a,
    0x510e527f, 0x9b05688c, 0x1f83d9ab, 0x5be0cd19
};

/* Helper: Right rotate */
static inline uint32_t rotr(uint32_t x, uint32_t n) {
    return (x >> n) | (x << (32 - n));
}

/* Helper: Right shift */
static inline uint32_t shr(uint32_t x, uint32_t n) {
    return x >> n;
}

/* Helper: Choose function */
static inline uint32_t ch(uint32_t x, uint32_t y, uint32_t z) {
    return (x & y) ^ (~x & z);
}

/* Helper: Majority function */
static inline uint32_t maj(uint32_t x, uint32_t y, uint32_t z) {
    return (x & y) ^ (x & z) ^ (y & z);
}

/* Helper: Sigma0 (large) */
static inline uint32_t sigma0(uint32_t x) {
    return rotr(x, 2) ^ rotr(x, 13) ^ rotr(x, 22);
}

/* Helper: Sigma1 (large) */
static inline uint32_t sigma1(uint32_t x) {
    return rotr(x, 6) ^ rotr(x, 11) ^ rotr(x, 25);
}

/* Helper: sigma0 (small) */
static inline uint32_t sigma0_small(uint32_t x) {
    return rotr(x, 7) ^ rotr(x, 18) ^ shr(x, 3);
}

/* Helper: sigma1 (small) */
static inline uint32_t sigma1_small(uint32_t x) {
    return rotr(x, 17) ^ rotr(x, 19) ^ shr(x, 10);
}

void sha256_init(sha256_context *ctx) {
    memcpy(ctx->state, sha256_initial_state, sizeof(sha256_initial_state));
    memset(ctx->buffer, 0, sizeof(ctx->buffer));
    ctx->total_len = 0;
    ctx->finalized = false;
}

void sha256_update(sha256_context *ctx, const uint8_t *data, uint32_t len) {
    uint32_t offset, need;

    if (ctx->finalized) return;

    /* Process existing buffer first */
    offset = ctx->total_len % SHA256_BLOCK_SIZE;
    need = SHA256_BLOCK_SIZE - offset;

    if (len < need) {
        memcpy(ctx->buffer + offset, data, len);
        ctx->total_len += len;
        return;
    }

    /* Fill buffer and process */
    memcpy(ctx->buffer + offset, data, need);
    ctx->total_len += need;

    /* Process this block */
    uint32_t w[64];
    uint32_t a, b, c, d, e, f, g, h;
    uint32_t t1, t2;

    /* Copy block into W[0..15] */
    for (int i = 0; i < 16; i++) {
        w[i] = ((uint32_t)ctx->buffer[i * 4] << 24) |
               ((uint32_t)ctx->buffer[i * 4 + 1] << 16) |
               ((uint32_t)ctx->buffer[i * 4 + 2] << 8) |
               ((uint32_t)ctx->buffer[i * 4 + 3]);
    }

    /* Expand W[16..63] */
    for (int i = 16; i < 64; i++) {
        w[i] = sigma1_small(w[i - 2]) + w[i - 7] +
               sigma0_small(w[i - 15]) + w[i - 16];
    }

    /* Initialize working variables */
    a = ctx->state[0];
    b = ctx->state[1];
    c = ctx->state[2];
    d = ctx->state[3];
    e = ctx->state[4];
    f = ctx->state[5];
    g = ctx->state[6];
    h = ctx->state[7];

    /* Main compression loop */
    for (int i = 0; i < 64; i++) {
        t1 = h + sigma1(e) + ch(e, f, g) + sha256_k[i] + w[i];
        t2 = sigma0(a) + maj(a, b, c);
        h = g;
        g = f;
        f = e;
        e = d + t1;
        d = c;
        c = b;
        b = a;
        a = t1 + t2;
    }

    /* Update hash state */
    ctx->state[0] += a;
    ctx->state[1] += b;
    ctx->state[2] += c;
    ctx->state[3] += d;
    ctx->state[4] += e;
    ctx->state[5] += f;
    ctx->state[6] += g;
    ctx->state[7] += h;

    /* Process remaining data */
    data += need;
    len -= need;

    while (len >= SHA256_BLOCK_SIZE) {
        memcpy(ctx->buffer, data, SHA256_BLOCK_SIZE);
        ctx->total_len += SHA256_BLOCK_SIZE;

        /* Process block (same as above) */
        for (int i = 0; i < 16; i++) {
            w[i] = ((uint32_t)ctx->buffer[i * 4] << 24) |
                   ((uint32_t)ctx->buffer[i * 4 + 1] << 16) |
                   ((uint32_t)ctx->buffer[i * 4 + 2] << 8) |
                   ((uint32_t)ctx->buffer[i * 4 + 3]);
        }
        for (int i = 16; i < 64; i++) {
            w[i] = sigma1_small(w[i - 2]) + w[i - 7] +
                   sigma0_small(w[i - 15]) + w[i - 16];
        }

        a = ctx->state[0];
        b = ctx->state[1];
        c = ctx->state[2];
        d = ctx->state[3];
        e = ctx->state[4];
        f = ctx->state[5];
        g = ctx->state[6];
        h = ctx->state[7];

        for (int i = 0; i < 64; i++) {
            t1 = h + sigma1(e) + ch(e, f, g) + sha256_k[i] + w[i];
            t2 = sigma0(a) + maj(a, b, c);
            h = g;
            g = f;
            f = e;
            e = d + t1;
            d = c;
            c = b;
            b = a;
            a = t1 + t2;
        }

        ctx->state[0] += a;
        ctx->state[1] += b;
        ctx->state[2] += c;
        ctx->state[3] += d;
        ctx->state[4] += e;
        ctx->state[5] += f;
        ctx->state[6] += g;
        ctx->state[7] += h;

        data += SHA256_BLOCK_SIZE;
        len -= SHA256_BLOCK_SIZE;
    }

    /* Store remaining data in buffer */
    memcpy(ctx->buffer, data, len);
    ctx->total_len += len;
}

bool sha256_final(sha256_context *ctx, uint8_t *digest) {
    if (ctx->finalized) return false;

    uint32_t msg_len = ctx->total_len;
    uint32_t offset = msg_len % SHA256_BLOCK_SIZE;
    uint32_t pad_len;

    /* Padding: 0x80 byte followed by zeros and length */
    if (offset < 56) {
        pad_len = 56 - offset;
    } else {
        pad_len = 56 + (64 - offset);
    }

    /* Add 0x80 byte */
    ctx->buffer[offset] = 0x80;
    offset++;

    /* Add padding zeros */
    memset(ctx->buffer + offset, 0, pad_len - 1);

    /* Add original length in bits (big-endian) */
    uint64_t bit_len = (uint64_t)msg_len * 8;
    for (int i = 0; i < 8; i++) {
        ctx->buffer[pad_len + i] = (uint8_t)(bit_len >> (56 - i * 8));
    }

    /* Process final block */
    uint32_t w[64];
    uint32_t a, b, c, d, e, f, g, h;
    uint32_t t1, t2;

    for (int i = 0; i < 16; i++) {
        w[i] = ((uint32_t)ctx->buffer[i * 4] << 24) |
               ((uint32_t)ctx->buffer[i * 4 + 1] << 16) |
               ((uint32_t)ctx->buffer[i * 4 + 2] << 8) |
               ((uint32_t)ctx->buffer[i * 4 + 3]);
    }
    for (int i = 16; i < 64; i++) {
        w[i] = sigma1_small(w[i - 2]) + w[i - 7] +
               sigma0_small(w[i - 15]) + w[i - 16];
    }

    a = ctx->state[0];
    b = ctx->state[1];
    c = ctx->state[2];
    d = ctx->state[3];
    e = ctx->state[4];
    f = ctx->state[5];
    g = ctx->state[6];
    h = ctx->state[7];

    for (int i = 0; i < 64; i++) {
        t1 = h + sigma1(e) + ch(e, f, g) + sha256_k[i] + w[i];
        t2 = sigma0(a) + maj(a, b, c);
        h = g;
        g = f;
        f = e;
        e = d + t1;
        d = c;
        c = b;
        b = a;
        a = t1 + t2;
    }

    ctx->state[0] += a;
    ctx->state[1] += b;
    ctx->state[2] += c;
    ctx->state[3] += d;
    ctx->state[4] += e;
    ctx->state[5] += f;
    ctx->state[6] += g;
    ctx->state[7] += h;

    /* Output digest (big-endian) */
    for (int i = 0; i < 8; i++) {
        digest[i * 4]     = (ctx->state[i] >> 24) & 0xFF;
        digest[i * 4 + 1] = (ctx->state[i] >> 16) & 0xFF;
        digest[i * 4 + 2] = (ctx->state[i] >> 8) & 0xFF;
        digest[i * 4 + 3] = ctx->state[i] & 0xFF;
    }

    ctx->finalized = true;
    return true;
}

void sha256_hash(const uint8_t *data, uint32_t len, uint8_t *digest) {
    sha256_context ctx;
    sha256_init(&ctx);
    sha256_update(&ctx, data, len);
    sha256_final(&ctx, digest);
}

bool sha256_compare(const uint8_t *a, const uint8_t *b) {
    return memcmp(a, b, SHA256_DIGEST_SIZE) == 0;
}

void sha256_print(const uint8_t *digest) {
    for (int i = 0; i < SHA256_DIGEST_SIZE; i++) {
        printf("%02x", digest[i]);
    }
    printf("\n");
}
