// Package pipeline implements a simplified CI/CD pipeline framework.
//
// CI/CD (Continuous Integration / Continuous Deployment) is a software
// development practice where code changes are automatically built, tested,
// and deployed to production. This package provides the core types and
// mechanisms for defining and running pipeline workflows.
//
// Core concepts:
//   - Pipeline: A sequence of stages that process code changes
//   - Stage: A logical phase (build, test, deploy) containing steps
//   - Step: An individual task within a stage (compile, run tests, etc.)
//   - Trigger: An event that starts a pipeline (commit, tag, schedule)
//   - Artifact: Build outputs (binaries, test reports, coverage data)
//   - Webhook: HTTP callbacks for external integration
package pipeline
