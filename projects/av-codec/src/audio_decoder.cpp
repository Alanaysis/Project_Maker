...
int AudioDecoder::init(const AudioDecoderConfig& config) {
    if (initialized_) {
...
int AudioDecoder::decode(const AVPacket* pkt, AVFrame* frame) {
    if (!initialized_) {
...
int AudioDecoder::flush(AVFrame* frame) {
    if (!initialized_) {
        return -1;
    }
...
const char* AudioDecoder::getName() const {
