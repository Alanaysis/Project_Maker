...
class MiniBatchKMeans:
    ...
    适用于大规模数据集的 K-Means 变体，通过小批量数据更新中心点，
    大幅减少计算时间，同时保持较好的聚类质量。

    参数:
        n_clusters: 簇的数量，默认为 3
        batch_size: 每批次的样本数，默认为 100
        max_iter: 最大迭代次数，默认为 100
        tol: 收敛阈值，默认为 1e-4
        init: 初始化方法，'random' 或 'kmeans++'，默认为 'random'
        random_state: 随机种子，默认为 None
        n_init: 使用不同种子运行算法的次数，默认为 1
    """
    ...
    def _init_centroids_kmeans_plus_plus(self, X, rng):
        ...
        for k in range(1, self.n_clusters):
            # 计算每个点到最近中心的距离
            distances = np.min([np.sqrt(np.sum((X - centroids[i]) ** 2, axis=1))
                               for i in range(k)], axis=0)
    ...
    def _get_mini_batch(self, X, rng):
        """获取一个小批量数据"""
        n_samples = X.shape[0]
    ...
    def fit(self, X):
        ...
        for run in range(self.n_init):
            ...
            for iteration in range(self.max_iter):
                ...
                # 获取小批量数据
                batch_X, batch_indices = self._get_mini_batch(X, rng)

                # 分配簇
                batch_labels = self._assign_clusters(batch_X, centroids)
                ...
                for k in range(self.n_clusters):
                    ...
                    if np.any(mask):
                        # 计算新样本的均值
                        new_points = batch_X[mask]

                        # 增量更新公式
                        cluster_counts[k] += len(new_points)
                        eta = 1.0 / cluster_counts[k]
                        centroids[k] = (1 - eta) * centroids[k] + eta * np.mean(new_points, axis=0)
            ...
            # 计算完整的 WCSS
            labels = self._assign_clusters(X, centroids)
            inertia = self._compute_wcss(X, labels, centroids)
