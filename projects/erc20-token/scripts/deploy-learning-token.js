/**
 * Deployment script for LearningToken (custom ERC20 implementation)
 *
 * Usage:
 *   npx hardhat run scripts/deploy-learning-token.js
 *   npx hardhat run scripts/deploy-learning-token.js --network sepolia
 */
...
async function main() {
  ...
  console.log("LearningToken deployed to:", address);
  console.log("Token name:", await token.name());
  console.log("Token symbol:", await token.symbol());
  console.log("Total supply:", ethers.formatEther(await token.totalSupply()), "LT");
  ...
  if (conn.networkName !== "default" && conn.networkName !== "hardhat" && conn.networkName !== "localhost") {
    ...
    try {
      await hre.run("verify:verify", {
        address: address,
        constructorArguments: ["Learning Token", "LT", 18, 1000000],
      });
