 ...
 *
 * Comprehensive set of easing functions for animations.
 * Each function maps progress t (0-1) to eased progress (0-1).
 */
...
// === Bounce ===
const bounceOut: EasingFunction = (t: number): number => {
  const n1 = 7.5625;
  const d1 = 2.75;
  if (t < 1 / d1) {
    return n1 * t * t;
...
export const easeInBounce: EasingFunction = (t: number): number => 1 - bounceOut(1 - t);
...
export function resolveEasing(easing: EasingFunction | string | undefined): EasingFunction {
  ...
  if (!fn) {
    console.warn(`Unknown easing function: "${easing}", falling back to ${DEFAULT_EASING.name}`);
    return DEFAULT_EASING;
  }
  return fn;
}
